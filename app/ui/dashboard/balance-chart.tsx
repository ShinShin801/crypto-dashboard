...
export default async function BalanceChart() {
  ...
  const { yAxisLabels, topLabel } = generateYAxis(balance);
          ...
          {balance.map((month) => (
            <div key={month.month} className="flex flex-col items-center gap-2">
              <div
                ...
                style={{
                  height: `${(chartHeight / topLabel) * month.matic_balance}px`,
                }}
              ></div>
              <p className="-rotate-20 sm:rotate- text-xs text-gray-400">
                {month.month}
              </p>
            </div>
          ))}
